...
document.querySelector('.btn-search').addEventListener('click', function() {
    const destination = document.querySelector('.search-input input[type="text"]').value;
    const date = document.querySelector('.search-input input[type="date"]').value;
    const people = document.querySelector('.search-input select').value;
...
document.querySelectorAll('.btn-book').forEach(button => {
    button.addEventListener('click', function() {
        ...
        const tourPrice = this.closest('.tour-card').querySelector('.tour-price').textContent;
        
        if (confirm(`Bạn có muốn đặt tour "${tourTitle}" với giá ${tourPrice}?`)) {
...
// Example: <span data-target="1000">0</span>
